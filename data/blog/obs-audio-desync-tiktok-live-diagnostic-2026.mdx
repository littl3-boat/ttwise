...
```mermaid
flowchart TD
    A["Start Stream Test"] --> B{"Delay From Start?"}
    B --|"Yes"|--> C{"Worse Over Time?"}
    B --|"No"|--> D["Perfect Sync Start"]
    D --> E{"Appears After 20m?"}
    E --|"Yes"|--> F["Progressive Drift (Sample Rate)"]
    C --|"Yes"|--> G["Compounding Lag (System Stress)"]
    C --|"No"|--> H["Static Offset (Processing Latency)"]
