...
```mermaid
graph TD
    subgraph "The Interaction Gap"
        A["Viewer Scrolls FYF"] --> B["Stops on your Gameplay"]
        B --> C{"Is the Streamer Talking?"}
        
        C -- "No (Silent Gaming)" --> D["Passive Viewing (Lurking)"]
        C -- "Yes (Narrating)" --> E["Engagement Potential"]
        
        D --> F["Scrolls Away (0-2 mins)"]
        
        E --> G{"Is there a 'Direct Hook'?"}
        G -- "No (General Talk)" --> D
        G -- "Yes (Specific Question)" --> H["Viewer Types in Chat"]
        
        H --> I["The Active Interaction Loop"]
    end
