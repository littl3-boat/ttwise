...
## The One Diagram That Ends the Argument

```mermaid
flowchart TD
  A[Consistent Live, No Growth] --> B{Do new viewers enter?}
  B -->|No| C[Distribution bottleneck]
  B -->|Yes| D{Do they stay 60+ seconds?}
  D -->|No| E[Retention bottleneck]
  D -->|Yes| F{Do you gain follows?}
  F -->|No| G[Conversion bottleneck]
  F -->|Yes| H[Scale: repeat + improve]

  C --> C1[Stability baseline + packaging reset]
  C1 --> C2[Entry test for 3 streams]
  C2 --> C3[Check restrictions + change topic framing]

  E --> E1[Fix friction: audio, lighting, pacing]
  E1 --> E2[Repeat loop every 3 minutes]
  E2 --> E3[Reduce dead-air tasks]

  G --> G1[Give a reason to follow]
  G1 --> G2[Pin schedule + promise]
  G2 --> G3[Make follow part of the game]
```

This diagram shows the real question. If viewers aren’t entering, you don’t fix that with “better conversation.” If viewers enter and leave, you don’t fix that by changing hashtags. You fix the bottleneck you actually have.
