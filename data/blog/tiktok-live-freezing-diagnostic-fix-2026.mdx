...
```mermaid
graph TD
    subgraph "The Diagnostic Flow"
        A["Stream Freezes"] --> B{"Is your PC/Phone Lagging?"}
        B -- "Yes" --> C["Hardware Overload (CPU/GPU)"]
        B -- "No" --> D{"Is the Preview in OBS/Studio Frozen?"}
        
        D -- "Yes" --> E["Encoder Crash (GPU Driver/Settings)"]
        D -- "No" --> F{"Are the Green Bars Moving?"}
        
        F -- "No" --> G["Network Drop (Upload Speed/ISP)"]
        F -- "Yes" --> H["TikTok Server/App Glitch"]
    end
