...
<TLDR>
  - **核心痛点**：在 TikTok Live Studio 中使用 OBS 虚拟摄像头（配合 Aitum Vertical）时，OBS 预览非常丝滑，但 TikTok 直播画面却出现严重掉帧、卡顿。
  - **搜索意图**：TikTok Live Studio 虚拟摄像头掉帧修复、OBS Aitum Vertical 卡顿解决方法、直播间画面不连贯。
  - **关键结论**：掉帧通常源于“编码资源冲突”。由于 OBS 和 TikTok Studio 默认可能都在抢占同一个 NVENC 硬件编码芯片，建议将 OBS 的虚拟摄像头输出设置为 x264（CPU 编码），而将 NVENC 留给 TikTok Studio 进行最后的推流编码。此外，确保两者的分辨率（如 1080x1920）和帧率（60fps）完全匹配，以消除重采样造成的延迟。
</TLDR>

<PostBanner
  src="/static/images/tiktok-live-studio-lag-virtual-camera-aitum-vertical-2025/banner.svg"
  alt="Fix TikTok Live Studio Lag with OBS Virtual Camera Fix Banner"
/>
...
![Virtual Camera Pipeline Flow](/static/images/tiktok-live-studio-lag-virtual-camera-aitum-vertical-2025/diagram-1.svg)
...
![Diagnostic Steps](/static/images/tiktok-live-studio-lag-virtual-camera-aitum-vertical-2025/explain-1.svg)

*How to isolate whether the lag is coming from OBS, the Virtual Camera output, or TikTok Live Studio itself.*
