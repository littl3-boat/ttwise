...
```mermaid
flowchart TD
    A[Viewer-Swipes-Stream] --> B{Visual-Hook-Good?}
    B -- No --> C[Instant-Leave]
    B -- Yes --> D{Audio-Hook-Present?}
    D -- No --> E[Leave-Boredom]
    D -- Yes --> F{Context-Hook-Clear?}
    F -- No --> G[Leave-Confusion]
    F -- Yes --> H[Viewer-Stays]
```
