...
```mermaid
    graph TD
    A["OBS preview smooth, TikTok Live lags"] --> B{"Using a single PC?"}
    B --|"Yes"|--> C["Check GPU Load (Task Manager)"]
    B --|"No"|--> D["Check Capture Card / Network"]
    C --|"99%+"|--> E["Cap In-Game FPS"]
    C --|"<90%"|--> F["Run OBS as Admin"]
    F --> D
