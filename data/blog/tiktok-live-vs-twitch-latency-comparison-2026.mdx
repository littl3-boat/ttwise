...
```mermaid
graph TD
    subgraph "The Latency Pipeline"
        A["Streamer (OBS/Studio)"] -- "RTMP Ingest" --> B["Platform Ingest Server"]
        
        B -- "Twitch (Low Latency HLS)" --> C1["Proprietary CDN"]
        C1 -- "2s - 5s Delay" --> D1["Viewer (Web/App)"]
        
        B -- "TikTok (Aggressive Transcoding)" --> C2["Global Edge Nodes"]
        C2 -- "5s - 12s Delay" --> D2["Viewer (Infinite Scroll)"]
    end
    
    style D1 fill:#9146FF,color:#fff
    style D2 fill:#00f2ea,color:#000
```
