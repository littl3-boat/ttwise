...
```mermaid
graph TD
    subgraph "The Passive Scrolling Funnel"
        A["Viewer Scrolls FYF"] --> B["Lands on your Live"]
        B --> C{"Is there a 'Hook'?"}
        
        C -- "No (Silent/Idle)" --> D["Scrolls Away (0s-3s)"]
        C -- "Yes (Action/Talking)" --> E["Stays (30s+)"]
        
        E --> F{"Does it feel 'Member-Only'?"}
        F -- "Yes (Inside Jokes)" --> G["Leaves (Doesn't belong)"]
        F -- "No (Inclusive)" --> H["Interacts (Like/Comment)"]
        
        H --> I["Follows (After 5-10 mins)"]
    end
