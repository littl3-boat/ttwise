...
> Source discussion: "Stream disconnects immediately" — r/OBS. Creators often report that their stream works fine on Twitch but enters a "disconnect/reconnect" loop on TikTok Live.

---

## TL;DR

If OBS keeps disconnecting from TikTok Live, your bitrate is likely spiking above the **ingest limit** (even if set to 6000 Kbps) or your **Keyframe Interval** is wrong. Fix it by enforcing **Strict CBR at 4000 Kbps**, setting **Keyframes to 2s**, and manually swapping your **Ingest Server URL**.
...
```mermaid
graph TD
  A[Start Stream] --> B{What Happens?}
  B -->|Green Square -> Red Square| C[Connection Unstable - Network/ISP]
  B -->|Immediate Disconnect/Reconnect| D[Ingest Rejection - Protocol Error]
  B -->|Stream works but lags| E[Bitrate/Encoding Issue]
  D --> F{Check Keyframe Interval}
  F -->|Not 2s?| G[Set to 2s - Mandatory]
  F -->|Is 2s?| H{Check Bitrate Spikes}
  H -->|VBR or High CBR?| I[Force Strict CBR 3500-4000]
  H -->|Low CBR?| J[Change Ingest Server URL]
